// Clases predefinidas.
// digito \d, palabra \w, espacios \s
// digit \d, word \w (also digits), space \s


// 1
// 12
// 123
// 1234
// 12345
// 12345678910
// 12345678910a
// 13453243
// url: https://www.instagram.com/p/BXB4zsUlW5Z/?taken-by=beco.mx
// url: http://instagram.com/p/blablablah
// url: http://itam.mx/test
// http://instagram.com/p/blablablah
// csv1,csv2,csv3,csv4,csv5
// Rodrigo Jiménez Gutiérrez
// 5556581111
// 56-58-11-12
// 302-345-9876
// esto.es.un.mail+gmail@mail.com
// dominio.com
// [LOG ENTRY] [ERROR] The system is unstable
// [LOG ENTRY] [WARN] The system may be down
// [LOG ENTRY] [LOG] Everything is OK
// [LOG ENTRY] [LOG] [user:@beco] Logged in
// [LOG ENTRY] [LOG] [user:@beco] Clicked here
// [LOG ENTRY] [LOG] [user:@beco] Rated the app
// [LOG ENTRY] [LOG] [user:@beco] Logged out

// \d encuentra todos los digitos (0-9)

const string = 'Mi numero telefonico es 5678919';

//digitos
// console.log(string.match(/\d/));
// console.log(string.match(/\d+/));
// console.log(string.match(/\d\d\d/));
console.log(string.match(/\d/g));

//palabras \w icluye digitos
// console.log(string.match(/\w/));
// console.log(string.match(/\w+/));
// console.log(string.match(/\w\w\w/));
console.log(string.match(/\w/g));

//espacios \s
// console.log(string.match(/\s/));
// console.log(string.match(/\s+/));
// console.log(string.match(/\s\s\s/));
console.log(string.match(/\s/g));